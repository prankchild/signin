...
export default {
  ...
  methods: {
    ...
    timeFormat(iTime) {
      // var JsonDateValue = new Date( + iTime + 8*3600*1000); 
      // var text = JsonDateValue.toISOString().replace(/T/g, ' ').replace(/\.[\d]{3}Z/, '');
      var date = new Date(iTime.addtime).toISOString().replace(/T/g, ' ').replace(/\.[\d]{3}Z/, '')
    return date
    }
  },
}
