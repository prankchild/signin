  ...
  methods: {
    ...
    async getData(){
      ...
      if( res.status === 200 ){
        ...
        this.time = this.mettingdRecord[0].time
        console.log(this.mettingdRecord )
      }
    },
    timeFormat(iTime) {
      // var time = "2021-07-19T08:35:32.000Z"
      // console.log(time);.toISOString() +new Date()+
      var date = iTime.replace(/T/g, ' ').replace(/\.[\d]{3}Z/, '')
      console.log(date);
      var date = new Date(8*3600*1000).toISOString().replace(/T/g,' ').replace(/\.[\d]{3}Z/,'');
      return date
    }
  },
}
