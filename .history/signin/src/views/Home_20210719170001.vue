<template>
  <div class="home">
    ...
    <div class="from">
      ...
      />
      <div class="divs" style="margin: 16px;">
  ...
  methods: {
    async onSubmit(values) {
        ...
        if(res.status === 200){
          ...
          this.$notify({ type: "danger", message: "签到失败,服务器出现问题" });
        }
    },
    async newMeeting(){
      const { data:res } = await this.$http.get("/User/getMeeting")
      console.log(res);
      if(res.status === 200){
        this.meeting = res.meetingname
      }else{
        this.meeting = '会议签到'
      }
    },
